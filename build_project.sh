#! /bin/bash

#node build.js service-source
#node build.js service-example-source-book
#node build.js service-example-source-journal
#node build.js service-example-proposal-source-book
#node build.js service-example-proposal-source-journal
#node build.js service-example-keyword
#node build.js service-example-proposal-keyword
#node build.js service-example-translation
#node build.js service-example-proposal-translation

node build.js ui-model-editable
node build.js ui-model-source-type

node build.js ui-component-source-base
node build.js ui-component-example-keyword
node build.js ui-component-example-translation
node build.js ui-component-example-source-book
node build.js ui-component-example-source-journal
node build.js ui-component-example-proposal-keyword
node build.js ui-component-example-proposal-translation
node build.js ui-component-example-proposal-source-book
node build.js ui-component-example-proposal-source-journal

node build.js ui-model-source-base
node build.js ui-model-source-base-default-impl

node build.js ui-directive-editable
node build.js ui-directive-list-manipulator

node build.js ui-model-example-keyword
node build.js ui-model-example-keyword-default-impl
node build.js ui-model-example-translation
node build.js ui-model-example-translation-default-impl
node build.js ui-model-example-source-book
node build.js ui-model-example-source-book-default-impl
node build.js ui-model-example-source-journal
node build.js ui-model-example-source-journal-default-impl
node build.js ui-model-example-proposal-keyword
node build.js ui-model-example-proposal-keyword-default-impl
node build.js ui-model-example-proposal-translation
node build.js ui-model-example-proposal-translation-default-impl
node build.js ui-model-example-proposal-source-book
node build.js ui-model-example-proposal-source-book-default-impl
node build.js ui-model-example-proposal-source-journal
node build.js ui-model-example-proposal-source-journal-default-impl
node build.js ui-model-list-example-proposal-keyword
node build.js ui-model-list-example-proposal-keyword-default-impl
node build.js ui-model-list-example-proposal-translation
node build.js ui-model-list-example-proposal-translation-default-impl

